...
class EEGApp(App):
    ...
    def on_button_pressed(self, event: Button.Pressed):
        if event.button.id == "start":
            ...
            self.stop_recording()
        elif event.button.id == "impedance":
            self.run_impedance_check()
    ...
    def start_recording(self):
        ...
        # params.serial_port = autodetect_cyton_port()
    ...
    def update_display(self):
        ...
        pkg = latest[PACKAGE_CHANNEL]

        if len(pkg) > 0:
            current = int(pkg[-1])

            if state.last_packet is not None:
                expected = (state.last_packet + 1) % 256
                if current != expected:
                    diff = (current - expected) % 256
                    state.lost_packets += diff

            state.last_packet = current
        lines = []
        for i, ch in enumerate(eeg,start=1):
            val = np.mean(ch)
            railing = "⚠️" if abs(val) > RAIL_THRESHOLD else ""
            lines.append(f"Ch{i:02d}: {val: .5f} {railing}")

        lines.append(f"Packet loss: {state.lost_packets}")
        self.query_one("#channels").update("\n".join(lines))

import serial.tools.list_ports

def autodetect_cyton_port():
    ...
    raise RuntimeError("No Cyton-compatible serial port found")

def run_impedance_check(self):
    if state.board is None:
        self.query_one("#status").update("Board not initialized")
        return

    self.query_one("#status").update("Running impedance check...")
    state.impedance = [0] * 16

    threading.Thread(target=self._impedance_loop, daemon=True).start()


def _impedance_loop(self):
    try:
        for ch in range(1, 17):  # Cyton channels are 1-based
            # Enable test signal on channel
            cmd = f"z{ch}"
            state.board.config_board(cmd)
            ...
            # crude impedance proxy (variance / amplitude)
            impedance = np.std(ch_data)
            state.impedance[ch - 1] = impedance

            print(f"Ch{ch} impedance approx: {impedance:.2f}")

            # Disable test signal
            state.board.config_board("Z")

        self.query_one("#status").update("Impedance check done")

    except Exception as e:
        self.query_one("#status").update(f"Impedance error: {e}")
