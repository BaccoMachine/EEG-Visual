...
def csv2exps(datadir): #get all csvs in a data diretory, return a dictionary of experiments and a list of all them
    #datadir = Path('data')
    allf = {}
    exps = {} 
    sgms = []
    ...
    for files in datadir.glob('*.csv'):
        ...
        sgms.extend(file_exps) #add segm to global list
        exps[files.stem] = file_exps
    return  exps, sgms

    #exps = {'filename': [ exp1, exp2, ...], 'filename2': [ exp1, exp2, ...], ...} where expi is a repeated original experiment
    #sgms = [exp1, exp2, ...] 


def sampling_frequency(df): #get sampling freq. Reminder: timestamp depnds on the writing of the file not the actual sampling frequency of the devicee
...
def export_spettrogrammi_completi(sgms, outdir, fs_hardware=125.0):
    ...
    for i, exexp in enumerate(sgms):
        print(f"Esportazione 16 canali - Esperimento {i+1}...")
        ...
        fig.suptitle(f'Esperimento {i+1} (Dettaglio 16 Canali)\nDurata: {lunghezza_sec:.1f}s', fontsize=22, fontweight='bold', y=0.99)
        ...
        for ch_idx, chn in enumerate(chns):
            ...
            for nome, (f_min, f_max) in bande_eeg.items():
                ...
                if ch_idx == 0: # Scrive i simboli greci SOLO sul primo canale in alto
                    ax.text(lunghezza_sec * 0.01, (f_min + f_max)/2, nome, color='white', alpha=0.8, va='center', fontsize=10, fontweight='bold')
        ...
        axes[-1].set_xlabel('Tempo dall\'inizio (Secondi)', fontsize=18, fontweight='bold')
        ...
        filepath = outdir / f"Exp_{i+1:02d}_Dettaglio_16_canali.png"
...
def export_spettrogramma_medio(sgms, outdir, fs_hardware=125.0):
    ...
    for i, exexp in enumerate(sgms):
        print(f"Esportazione Media Canali - Esperimento {i+1}...")
        lunghezza_sec = len(exexp) / fs_hardware
        marker_times = estrai_tempi_marker(exexp, fs_hardware)
        ...
        ax.set_title(f'Esperimento {i+1} (MEDIA dei 16 Canali EEG)\nDurata: {lunghezza_sec:.1f}s', fontsize=18, fontweight='bold')
        ax.set_ylabel('Frequenza (Hz)', fontsize=14, fontweight='bold')
        ax.set_xlabel('Tempo dall\'inizio (Secondi)', fontsize=14, fontweight='bold')
        ...
        for nome, (f_min, f_max) in bande_eeg.items():
            ax.axhline(f_min, color='white', linestyle=':', alpha=0.4) # Linea di demarcazione inferiore
            ax.text(lunghezza_sec * 0.01, (f_min + f_max)/2, nome, color='white', alpha=0.8, va='center', fontsize=12, fontweight='bold')
        ...
        cbar.set_label('Potenza Media (dB)', fontsize=12)

        plt.tight_layout()
        
        filepath = outdir / f"Exp_{i+1:02d}_Media_Globale.png"
...
def export_welch_per_sezione(sgms, outdir, fs_hardware=125.0):
    ...
    for i, exexp in enumerate(sgms):
        print(f"Welch per sezioni - Esperimento {i+1}...")
        ...
        ax.set_title(f'Esperimento {i+1} — Welch per Sezione', fontsize=15, fontweight='bold')
        ax.legend(fontsize=11)
        ax.set_xlim(1, 40)

        plt.tight_layout()
        filepath = outdir / f"Exp_{i+1:02d}_Welch_Sezioni.png"
        plt.savefig(filepath, dpi=150, bbox_inches='tight')
        plt.close(fig)
...
if __name__ == "__main__":
    ...
    print("Lettura file CSV e frammentazione esperimenti...")
    esperimenti_dizionario, segmenti_lista = csv2exps(datadir)
    
    if segmenti_lista:
        print(f"Trovati {len(segmenti_lista)} frammenti. Inizio generazione immagini...")
        #export_spettrogramma_medio(segmenti_lista, outdir, fs_hardware=125.0)
        #xport_spettrogrammi_completi(segmenti_lista, outdir, fs_hardware=125.0)
        export_welch_per_sezione(segmenti_lista, outdir, fs_hardware=125.0)
        print(f"Tutto completato! Controlla la cartella: {outdir.absolute()}")
    else:
        print("Nessun segmento valido trovato. Controlla i file in /data.")